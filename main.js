...
global.reloadHandler = function (restatConn) {
  ...
  conn.welcome = 'ようこそ Youkuso, @user!\nSelamat datang di @subject.\n\n@desc'
  conn.bye = '左様なら Sayounara, @user!'
...
global.reload = (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (err) conn.logger.error(`SyntaxError di '${filename}'\n${err}`)
    else try {
...
async function _quickTest() {
  ...
  if (!s.ffmpeg) conn.logger.warn('Tidak terdapat ffmpeg. Harap install ffmpeg.')
  if (s.ffmpeg && !s.ffmpegWebp) conn.logger.warn('Stickers may not animated without libwebp on ffmpeg (--enable-ibwebp while compiling ffmpeg)')
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('Tidak terdapat imagemagick. Harap install imagemagick.')
}
